...
| IPC 处理 | ~619-1272 | 41个 IPC 通道（设置/主题/拨号/设备/云端） |
...
**服务器列表遍历**：按 `cloudServers` 列表顺序尝试连接，成功后停止遍历。

### 2.4 设备连接管理（PhoneConnectionManager）

独立模块，管理最多 2 台手机的双通道连接。
...
window.api = {
  send: async function(channel, ...args) {
    switch (channel) {
      case 'dial': return window.go.main.App.SendDial(args[0]);
      case 'hangup': return window.go.main.App.SendHangup();
      // ...
    }
  },
  invoke: async function(channel, ...args) { /* ... */ },
  on: function(channel, callback) { /* 事件注册 */ }
};
```
...
| 前端事件 | IPC 双向事件驱动 | Wails 绑定 + 500ms 轮询模拟事件 |
| 号码校验 | 正则 `/^(\+?[\d\s\-\(\)]{4,20})$/` | `isValidDialNumber()` 逐字符检查 |
| 设备注释 key | `pin\|name` | `pin`（仅用 PIN） |
| 悬浮条 | 独立窗口 440×48 | 窗口缩放为 400×52 |
| 连接上限 | 2 台 | 10 台 |
| TCP KeepAlive | `socket.setKeepAlive(true, 10000)` | `TCPConn.SetKeepAlivePeriod(10s)` |
