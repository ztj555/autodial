...
func handleLocalWS(conn *websocket.Conn) {
	...
	for {
		...
		case "phone_hello":
			...
			if len(pin) < 4 {
				conn.WriteJSON(map[string]string{"type": "auth_fail", "reason": "invalid pin"})
				fileLog("W", "WS", "", fmt.Sprintf("auth fail: short pin from %s", clientIP))
				continue
			}
			if pin != pinCode {
				conn.WriteJSON(map[string]string{"type": "auth_fail", "reason": "pin mismatch"})
				fileLog("W", "WS", "", fmt.Sprintf("auth fail: pin mismatch from %s (got=%s want=%s)", clientIP, pin, pinCode))
				continue
			}
			if name == "" {
