...
func connectCloudServer(serverURL string) {
	...
	if err != nil {
		...
		if err2 != nil {
			...
			cloudWsMu.Unlock()
			scheduleCloudReconnect() // Fix D3: retry even on bad URL so user can fix config
			return
		}
		u = u2
	}
	...
	// Fix B1: read pinCode under lock
	...
	go func(conn *websocket.Conn, myGen int) {
		defer func() {
			conn.Close()
			cloudWsMu.Lock()
			isCurrentGen := cloudWs == conn && cloudGeneration == myGen
			if isCurrentGen {
				cloudWs = nil
				cloudConnected = false
			}
			cloudWsMu.Unlock()
			// Fix B2: only clean up and reconnect if this is the current generation.
			// Old generations' defer must not touch active connection state.
			if isCurrentGen {
