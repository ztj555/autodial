...
构建完成后，输出在：`pc-app\output\AutoDial-win32-x64\AutoDial.exe`
...
构建完成后，exe在：`cloud-relay\python\dist\cloud_relay_v2.exe`

**启动云端服务：**
```bash
# 运行启动脚本（需要管理员权限）
cloud-relay\start.bat
```
...
### 首次使用
1. 手机端安装APK并授予必要权限
2. PC端运行启动脚本：`pc-app\启动.bat`
3. 手机端输入PC端显示的4位配对码进行连接
4. 浏览器扩展自动检测到PC端后即可在CRM网页使用

### 本地构建PC端exe
1. 杀死正在运行的AutoDial进程：`taskkill /F /IM AutoDial.exe`
2. 设置Electron镜像源：`set ELECTRON_MIRROR=https://npm.taobao.org/mirrors/electron/`
3. 执行打包：`node pc-app/pack.js`
4. 构建完成后exe在：`pc-app\output\AutoDial-win32-x64\AutoDial.exe`
...
### Android端
- 项目路径: `android/` 或 `AutoDial-Android/`
...
- 项目路径: `pc-app/`
- 启动脚本: `pc-app\启动.bat`
- 本地构建脚本: `pc-app\build-local.bat`
- 主要文件: `main.js` (Electron主进程)
- 打包脚本: `pack.js`
- 构建输出: `pc-app\output\AutoDial-win32-x64\AutoDial.exe`
...
### 云端中继
- 项目路径: `cloud-relay/`
- 启动脚本: `cloud-relay\start.bat`
- Python版: `cloud-relay/python/cloud_relay_v2.py`
- Python构建脚本: `cloud-relay/python/build.bat`
- 构建输出: `cloud-relay/python/dist/cloud_relay_v2.exe`
