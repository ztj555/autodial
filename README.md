...
- **扩展 UI 对齐手机端**：Chrome 扩展天空蓝默认主题（v6.0 起为「16 套色相 × 亮白 / 暗夜 2 档」共 32 种外观，与手机端、PC 端同一份配色数据），修复状态文字着色、清理硬编码旧主题色
...
> **扩展端 v5.6 起**：远程列表只作为**候选池**，**不会自动切换**当前生效的云中继地址 ——
> 生效地址只由用户在弹窗/挂件里点「保存」决定，避免自动拉取把手动配置顶掉。
