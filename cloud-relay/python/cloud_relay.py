...
PHONE_TO_PC_TYPES = {'phone_hello', 'dial_result', 'sms_result', 'ack', 'ping'}
PC_TO_PHONE_TYPES = {'auth_ok', 'auth_fail', 'dial', 'sms', 'hangup', 'reconnect_request'}


def is_valid_pin(pin: str) -> bool:
    """校验配对码：4位纯数字 或 11位中国大陆手机号(1[3-9]xxxxxxxxx)"""
    if not pin or not pin.isdigit():
        return False
    if len(pin) == 4:
        return True
    if len(pin) == 11 and pin[0] == '1' and '3' <= pin[1] <= '9':
        return True
    return False
        ...
        async for raw in ws:
            ...
            if msg_type == 'phone_hello':
                ...
                if not is_valid_pin(pin):
                    await ws.send(json.dumps({'type': 'auth_fail', 'reason': '配对码无效（需4位或11位手机号）'}))
                    continue
            ...
            if msg_type == 'pc_hello':
                pin = msg.get('pin', '')
                if not is_valid_pin(pin):
                    await ws.send(json.dumps({'type': 'pc_auth_fail', 'reason': '配对码无效（需4位或11位手机号）'}))
                    continue
                remove_from_group(ws)
                meta['pin'] = pin
                meta['role'] = 'pc'
...
def update_tray_status(running):
    ...
    # 只更新标题和菜单，不修改图标对象 — 避免跨线程 pystray GUI 操作崩溃
    if tray_icon:
        try:
            tray_icon.title = f'AutoDial Cloud Relay\n{"Running" if running else "Stopped"} | Port {PORT}'
...
def create_menu():
    import pystray
    # 先快照数据，避免迭代时 asyncio 线程同时修改 pin_groups 导致 RuntimeError
    try:
        pc_count = sum(1 for g in list(pin_groups.values()) for _ in g.pcs)
        phone_count = sum(1 for g in list(pin_groups.values()) for _ in g.phones)
        group_count = len(pin_groups)
    except RuntimeError:
        pc_count = phone_count = group_count = 0
    ...
    return pystray.Menu(
        ...
        pystray.MenuItem(f'Port: {PORT}  |  Groups: {group_count}', None, enabled=False),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem(
...
def run_tray():
    ...
    tray_icon = pystray.Icon(
        'AutoDial Cloud Relay',
        icon=create_tray_icon(),  # 始终用绿色图标，状态看标题和菜单
