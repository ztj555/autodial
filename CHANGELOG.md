...
## 2026-09-18（下钻弹窗改为二级明细页 · v4.36）

> 需求：昨天做的那两个下钻**弹窗**改成**二级页面**，右上角加个返回就行。
> 只动 `cloud-relay/python/dashboard.html` 一个文件，**服务端一行未改，配色一行未动**。

**一、弹窗 → 二级页**

原来点卡片是浮层压在首页上（有遮罩、表格锁在 320px 的内滚动框里）；现在是整块切页：

- 新增 `#page-dash-detail`，原弹窗内容整体迁入，**`stat-detail-*` 这些 id 全部沿用**，
  所以渲染逻辑（`renderStatDetail` / `renderOpsDetail` / 画图）**一行都没改**。
- 遮罩层与弹窗壳体（`#stat-detail-modal`）已删除；`closeStatDetail()` 保留为「返回首页」的别名。
- 顺着页面尺寸把弹窗里挤着的限制放开：表格去掉 320px 内滚动（跟整页滚）、趋势图 148px → 230px、
  KPI 卡最小宽度 118px → 150px（宽屏一行排开更舒展）。

**二、顶栏右上角「返回」**

- 只在二级明细页出现，切到其他 7 个页面自动隐藏（挂在 `showPage` 里统一切换）。
- 返回 = 回首页总览，**不重新拉数据**（区间汇总还在内存里），秒回且**时间区间保持不变**。
- 顶栏标题跟着明细走（「今日拨号数 · 按业务员」→ 返回后还原「首页总览」）。
- 侧边栏「首页总览」在明细页保持高亮（二级页没有独立入口，否则整列都不选中）。

**三、路由**

- 明细页 hash 形如 `#/dash-detail/dials`，**刷新不会掉回首页**；未登录时回首页（否则必然 401）。
- 进二级页用 `pushState`：**浏览器后退键 = 返回首页**；一级页之间仍是 `replaceState`。
- 新增 `popstate` 监听把 hash 解析回页面；**Esc 键**在无弹窗时也是返回首页。

**四、明细页自带时间区间**

顶部也放了一排「今天 / 昨天 / 近一周 / 本月」，两排 tab 由 `syncDashRangeUI()` 同步高亮，
在明细页直接切区间就能换数据，不必退回首页再点。运维类（连接数趋势 / 消息类型分布）依旧**不参与**
区间联动 —— 它们取的是服务进程内存数据，与时间段无关。

**五、验收**

`verify_home.js` 扩到 **73 项断言全绿**（原 40 项全部保留并改写为「活动页」口径），新增覆盖：
首页不显示返回按钮 / 进明细页首页确实隐藏（不是浮层）/ 返回按钮出现 / hash 带类型 /
明细页区间 tab 存在且与首页同步 / 表格不再受 320px 限制 / 离开明细页后按钮消失 /
点返回回首页且区间保持 / 侧边栏高亮正确 / 浏览器后退键回首页 / Esc 回首页 /
刷新后仍停在明细页 / 明细页内切区间标题与数据跟着变。

> 注：v4.34（人员「分组」改「部门」）那次提交没写 CHANGELOG，记录曾停在 v4.33。
> v4.36 起 `APP_VERSION` 已由 `4.33` 一次性上调为 `4.36`，与 v4.35 / v4.36 的改动对齐。

## 2026-09-18（首页可下钻 + 时间区间选择 · v4.35）

> 需求：首页「今日拨号数 / 今日登记数」两张卡片，点一下要能看到**每个业务员的明细**；
> 首页再加一个**时间选择**（今天 / 昨天 / 近一周 / 本月），切了之后所有按时间统计的模块一起跟着变。
> 改动 `cloud_relay_v2.py` + `dashboard.html`，**配色一行未动**。

**一、新增 `GET /api/v1/dashboard/summary?range=<today|yesterday|week|month>`（管理员鉴权）**

一次返回这个区间内的全部首页数字，前端不再自己算：

| 字段 | 含义 |
|---|---|
| `dials_total` / `dials_connected` / `dials_duration` | 拨号总数 / 接通数 / 通话总时长 |
| `dials_by_advisor[]` | 每个业务员的 `name`、`pin`、`dials`、`connected`、`duration`、`devices[]` |
| `visits_total` / `visits_unsynced` | 登记总数 / 其中待同步 CRM 的条数 |
| `visits_by_advisor[]` | 每个业务员的 `name`、`pin`、`visits`、`unsynced` |
| `visit_types[]` | 来访事由分布（`type` + `count`） |
| `series[]` | 时间序列：区间 ≤ 2 天按**小时**归集，更长按**天**（`granularity` 告诉前端） |
| `date_from` / `date_to` / `label` | 区间起止与中文标签，供界面显示 |

🔴 **口径一律在服务端算，前端只显示**。归属表达式直接复用列表页那一套（通话认「手机主人」`phones.default_pin`、
上门认「接待顾问」`kefu_tel`），避免首页数字和列表页数字打架 —— 这是 v4.32 定下的规矩，这次继续守。
数字天然自洽：**各业务员小计之和 == 合计**，已纳入验收脚本断言。

**二、首页两张数字卡可点击下钻**

点卡片弹出明细弹窗，内容是「**按业务员**」+「**按时间分布**」：

- 拨号数弹窗：KPI 四项（拨号总数 / 接通数 / 接通率 / 通话总时长）、按小时或按天的柱状图、
  业务员表格（拨号数 / 接通 / 接通率 / 时长 / 占比条），**并且列出这个人名下的设备各拨了几次**（`devices[]`）。
- 登记数弹窗：KPI 四项（登记总数 / 待同步 CRM / 参与业务员 / 人均登记）、来访事由标签、业务员表格（登记数 / 待同步 / 占比条）。
- **表格里的业务员可以点**：跳到「通话记录」或「上门登记」页，并把日期区间与该顾问**预先筛好**，不用再手点一遍。
- 卡片右上角常驻「查看明细 ›」角标，`:hover` 时高亮；卡片可 Tab 聚焦 + 回车打开（键盘可达）。

**三、运维区两张图也做同款下钻**（「其他模块同理」）

「连接数趋势（最近 4 小时）」「消息类型分布」两张图同样加了角标与弹窗：
前者列出每个采样点的 总连接 / PC / 手机 与峰值时刻；后者列出各消息类型的条数与占比。
这两块数据来自服务进程内存（历史只留 4 小时、消息累计自进程启动），**与首页时间区间无关**，
所以切 tab 时不跟着变 —— 已在代码里显式隔开，避免联动出怪结果。

**四、时间选择**

首页顶部一排 tab：今天 / 昨天 / 近一周 / 本月。切换后「拨号数 / 登记数 / 导出登记按钮文案」一起变，
右上角显示当前区间（如 `近一周 · 2026-09-12 ~ 2026-09-18`）。选择记进 `localStorage`，刷新后保持。

**五、验收**

`cloud-relay/.devdata/verify_home.js`（Playwright，开发库专用，不进仓库）跑完 **40 项断言全绿**：
四个区间的标题 / tab 高亮 / 导出文案、卡片与接口数字一致、弹窗行小计之和等于合计、
下钻跳转后的日期与顾问筛选确实生效、弹窗开着切区间内容跟着刷新、无未捕获 JS 错误、无 >=400 请求。
另外 `ws_sim.py` 用真 WebSocket 以手机 + PC 身份握手发消息，验证手机链路本身是通的。

> 注：v4.34（人员「分组」改「部门」）那次提交没写 CHANGELOG，本条之前的记录停在 v4.33。
