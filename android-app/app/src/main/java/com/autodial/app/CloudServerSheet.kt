...
class CloudServerSheet(private val activity: Activity, private val onChanged: () -> Unit) : BottomSheetDialog(activity) {
    ...
    private fun action(label:String, click:()->Unit)=TextView(activity).apply {
        text=label; textSize=12f; gravity=Gravity.CENTER; setTextColor(Color.parseColor(colors.primary)); setPadding((12*dp).toInt(),(10*dp).toInt(),(12*dp).toInt(),(10*dp).toInt()); layoutParams=LinearLayout.LayoutParams(0,-2,1f).apply{marginStart=(4*dp).toInt();marginEnd=(4*dp).toInt()}; background=android.graphics.drawable.GradientDrawable().apply{setColor(Color.parseColor(colors.bg2));cornerRadius=12*dp;setStroke((1*dp).toInt(),Color.parseColor(colors.primary))}; setOnClickListener{click()}
    }
    private fun render(results:Map<String,Boolean> = emptyMap()) {
        ...
        ctrl.getServerList().forEach { entry ->
            list.addView(LinearLayout(activity).apply {
                orientation=LinearLayout.VERTICAL; setPadding((12*dp).toInt(),(10*dp).toInt(),(12*dp).toInt(),(10*dp).toInt()); layoutParams=LinearLayout.LayoutParams(-1,-2).apply{bottomMargin=(8*dp).toInt()}; background=android.graphics.drawable.GradientDrawable().apply{setColor(Color.parseColor(colors.bg2));cornerRadius=12*dp;if(entry.url==current)setStroke((2*dp).toInt(),Color.parseColor(colors.primary))}
                ...
                addView(LinearLayout(activity).apply{orientation=LinearLayout.HORIZONTAL;addView(action("设为当前"){activity.getSharedPreferences("autodial",0).edit().putString("cloud_server",entry.url).apply();onChanged();render()});addView(action("测速"){activity.lifecycleScope.launch{render(mapOf(entry.url to ctrl.testServer(entry.url)))}});addView(action("删除"){ctrl.removeServer(entry.url);onChanged();render()})})
            })
        }
    }
    ...
    private fun testAll(){activity.lifecycleScope.launch{val r=ctrl.testAllServers(ctrl.getServerList()).associate{it.first.url to it.second};render(r);Toast.makeText(activity,"测速完成",Toast.LENGTH_SHORT).show()}}
    private fun fetchServers(message:String){Toast.makeText(activity,message,Toast.LENGTH_SHORT).show();activity.lifecycleScope.launch{val servers=ctrl.fetchServerListFromGist();if(!servers.isNullOrEmpty()){ctrl.setServerList(servers);onChanged();render();Toast.makeText(activity,"已获取 ${servers.size} 台服务器",Toast.LENGTH_SHORT).show()}else Toast.makeText(activity,"获取失败，请检查网络",Toast.LENGTH_SHORT).show()}}
}
