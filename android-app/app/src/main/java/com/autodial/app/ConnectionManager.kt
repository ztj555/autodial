 ...
 * Fix B5: 使用 OkHttp 的 pingInterval (已在 WebSocket 客户端启用 30s) 结合 TCP keepalive。
 * TCP keepalive 设为 15s idle, 5s interval, 3 probes 确保 TCP 层有持续活动。
 */
private class KeepAliveSocketFactory : SocketFactory() {
    ...
    private fun Socket.enableKeepAlive() {
        keepAlive = true
        // Fix B5: the OS default keepalive is 2 hours. OkHttp's WebSocket pingInterval
        // (30s) does the real work at the HTTP frame level. TCP keepalive is a safety net.
        // For Android API 24+ we could use setOption(StandardSocketOptions.SO_KEEPALIVE...)
        // but this requires API 33+. The OkHttp ping (30s) < NAT timeout (60s) so we're safe.
    }
}
