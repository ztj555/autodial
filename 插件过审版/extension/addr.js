 ...
 *   5. 权限模型（v6.4）：内置地址池由 manifest 静态声明；用户自填的其它主机走
 *      chrome.permissions.request 运行时按需授权（见 popup.js ensureOrigin）
 *
 * 加载方式：
...
var AD_ADDR = (function () {
  ...
  // ─── 静态白名单与运行时授权（v6.4）────────────────────
  // 权限模型：内置地址池在 manifest 的 host_permissions 中静态声明，开箱即用；
  // 用户自填的其它主机不静态声明，改由 chrome.permissions.request 在用户点击
  // 「测试 / 保存」时按需申请（见 popup.js 的 ensureOrigin）。这样既满足最小权限，
  // 又保留了自定义地址的能力。
  //
  // ⚠️ 这里必须与 manifest 的 host_permissions 保持一致；Chromium 的匹配模式中
  //    「不写端口 = 匹配该主机任意端口」，故此处只比对 scheme + host。
  var STATIC_ORIGINS = [
    'http://127.0.0.1',
    'http://localhost',
    'http://101.34.65.254',
    'http://262ao85kz470.vicp.fun',
    'http://192.168.2.55',
    'http://192.168.3.75'
  ];

  // 地址 -> origin 匹配模式：'262ao85kz470.vicp.fun:55535' => 'http://262ao85kz470.vicp.fun/*'
  function originFor(addr) {
    var c = cleanAddr(addr);
    if (!c) return '';
    var m = c.match(/^(https?):\/\//i);
    var scheme = m ? m[1].toLowerCase() : 'http';
    var rest = m ? c.slice(m[0].length) : c;
    var host = rest.split('/')[0].split(':')[0].trim();
    if (!host) return '';
    return scheme + '://' + host + '/*';
  }

  // 需要运行时授权时返回要申请的 origin，已含静态权限则返回 ''
  function grantNeeded(addr) {
    var o = originFor(addr);
    if (!o) return '';
    var base = o.replace(/\/\*$/, '');
    return STATIC_ORIGINS.indexOf(base) === -1 ? o : '';
  }

  // 候选池里挑一个「已有静态权限」的地址作为自动生效地址。
  // 若整池都没权限（例如远端列表新增了未见过的服务器），回落到内置默认地址 ——
  // 避免自动切到未授权主机后所有请求静默失败。
  function pickAuto(list) {
    var arr = Array.isArray(list) ? list : [];
    for (var i = 0; i < arr.length; i++) {
      if (!grantNeeded(arr[i])) return arr[i];
    }
    return AD_DEFAULT_ADDR;
  }
    ...
    return new Promise(function (resolve) {
      chrome.storage.local.get([K_ADDR, K_SRC, K_LIST, K_LIST_AT], function (s) {
        ...
        if (manual) {
          ...
          addr = pickAuto(list); source = 'auto';
        } else {
